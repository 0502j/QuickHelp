...
class MY_Model extends CI_Model
{
    ...
    public function _prep_after_read($data, $multi = TRUE)
    {
        ...
        if (isset($this->return_as_dropdown) && $this->return_as_dropdown == 'dropdown') {
            if (!empty($this->_empty_text_field))
                $dropdown[''] = $this->_empty_text_field;

            foreach ($data as $row) {
                $dropdown[$row[$this->primary_key]] = ucfirst($row[$this->_dropdown_field]);
            }
            $data = $dropdown;
     ...
     */
    public function insert($data = NULL, $last_id = false)
    {
    ...
    public function as_dropdown($field = NULL, $empty_text = NULL)
    {
        if (!isset($field)) {
        ...
        $this->_empty_text_field = $empty_text;
    ...
    public function getS2Term($term)
    {
        return !empty($term) ? ('fuzzy(' . implode('%) & fuzzy(', explode(' ', preg_replace("/\s{2,}/", ' ', trim(mb_strtoupper($term, 'UTF-8'))))) . '%)') : false;
    }
